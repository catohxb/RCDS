...
#Created by X. Huang, SLAC, 10/6/2015
...
def powellmain(func,x0,step,Dmat0,tol=1.0E-5,maxIt=100,maxEval=1500):
	...
	while it<maxIt:
		...
		for ii in range(Nvar):
			dv=Dmat[:,ii]
			(x1,f1,a1,a2,xflist,ndf)=bracketmin(func,xm,fm,dv,step)
			nf += ndf
			#print([it, ii, a1,a2, f1])
			
			print("iter %d, dir %d: begin\t%d\t%f" %(it, ii, g_cnt,f1))
			(x1,f1,ndf)=linescan(func,x1,f1,dv,a1,a2,Npmin,xflist)
			nf += ndf
		...
		if f0<=ft or 2*(f0-2*fm+ft)*((f0-fm-dl)/(ft-f0))**2 >= dl:
			...
			if max(dotp)<0.9:
				...
				(x1,f1,a1,a2,xflist,ndf)=bracketmin(func,xm,fm,dv,step)
				nf += ndf
				print("iter %d, new dir %d: begin\t%d\t%f " %(it,k, g_cnt,f1))
				(x1,f1,ndf) = linescan(func,x1,f1,dv,a1,a2,Npmin,xflist)
				print("end\t%d : %f\n" %(g_cnt,f1))
				nf=nf+ndf
				fm=f1
...
def bracketmin(func,x0,f0,dv,step):
	'''bracket the minimum
	Created by X. Huang, 10/5/2016
	Input: 
		 func is function handle, 
	...
	a2 = step
	if f0>fm+g_noise*3: #no need to go in the negative direction
		a1=0
		a1 = a1 - am
		a2 = a2 - am
		xflist[:,0] -= am
...
def linescan(func,x0,f0,dv,alo,ahi,Np,xflist):
